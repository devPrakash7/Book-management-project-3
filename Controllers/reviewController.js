  ...
  try {
    ...
    if (!isValidRequestBody(requestBody))
      return res
        ...
        .send({
          status: false,
          message: "Invalid request parameters.Please provide review details",
        });
    ...
    if (!isValidRequestBody(bookId))
      return res
        ...
        .send({
          status: false,
          message: "Invalid request parameters.Please provide Book id",
        });
    ...
    if (!ratingRegex.test(rating))
      return res
        ...
        .send({
          status: false,
          message: " Rating should be on a scale of 1-5.",
        });
    ...
    if (!findBook)
      return res
        ...
        .send({
          status: false,
          message: "No document found with this book Id or it maybe deleted",
        });
...
const updateReview = async function (req, res) {
  try {
    ...
    if (!isValidRequestBody(data))
      return res
        ...
        .send({
          status: false,
          message:
            "Invalid request parameters.Please provide the fields that you want to update",
        });
    ...
    if (!isValidObjectId(reviewId))
      return res
        .status(400)
        .send({ status: false, message: "Please provide valid review Id" });
    ...
    if (!findBook)
      return res
        .status(404)
        .send({ status: false, message: "No book found or it maybe deleted" });
    ...
    const updatedReview = await reviewModel.findOneAndUpdate(
      { _id: reviewId },
      { reviewedBy: data.reviewedBy, rating: data.rating, review: data.review },
...
const deleteReview = async function (req, res) {
  try {
    ...
    if (!findReview)
      return res
        .status(404)
        .send({ status: false, message: "No review found with this Id" });
