...
const getBookReviews = async function (req, res) {
  try {
    ...
    if (!bookData)
      return res.status(404).send({
        status: false,
        message: "No book document found or it maybe deleted",
      });
    ...
    if (!findReview)
      return res.status(404).send({
        status: false,
        message: "No review document found or it maybe deleted",
      });
    ...
    return res
      .status(200)
      .send({ status: true, message: "Books list", data: bookWithReview });
  } catch (err) {
    return res
      .status(500)
      .send({ status: false, message: "Error", error: err.message });
...
const updateBook = async function (req, res) {
  try {
    ...
    if (!getbook)
      return res.status(404).send({
        status: false,
        message: "No document found or it maybe deleted",
      });
    ...
    if (duplicateTitle)
      return res.status(400).send({
        status: false,
        message: "The title you want to update is already updated",
      });
